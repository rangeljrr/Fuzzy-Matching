...
from _config import input_database,results_database, source_database
...
def roddys_fuzzy_main(input_file_path, source_file_path,write_results_path, use_token_reduction=True):
    ...
    for i in range(input_data.shape[0]):
        ...
        target = input_data.iloc[i]['Target']

        # Use token reduction
        if use_token_reduction == True:
            fuzzy_dataframe = source_data.copy()
            fuzzy_dataframe['score'] = fuzzy_dataframe['Target'].apply(lambda x: token_intersection(x,target))# <-- Will need to rename and redo this algorithm
        ...
        fuzzy_dataframe['score'] = fuzzy_dataframe['Target'].apply(lambda x: score_strings(x,target))
        ...
        del input_row['Target']
        
        match_row = source_data[source_data['Target'] == best_match['Target']]
        del match_row['Target']
